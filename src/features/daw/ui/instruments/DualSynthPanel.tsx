...
import { useDualSynthStore } from "@/features/daw/state/dual-synth.store";
import { DualSynthParams } from "@/core/audio-engine/sources/synth/dual-osc-synth";
...
const DualSynthPanelComponent = ({ trackId }: Props) => {
  ...
  const set = useCallback(
    (p: DualSynthParams) => setParams(trackId, p),
    [setParams, trackId]
  );
          ...
          >
            <div className="mb-2 grid grid-cols-[auto_auto_1fr_auto_auto_auto] items-center gap-2">
              ...
              <input
                ...
                onChange={(e) => {
                  const next = (params.envs ?? []).slice();
                  next[idx] = { ...mod, group: e.target.value };
                  set({ envs: next });
                }}
                placeholder="Group"
              />
              <input
                ...
                onChange={(e) => {
                  const next = (params.envs ?? []).slice();
                  next[idx] = { ...mod, macro: e.target.value };
                  set({ envs: next });
                }}
                placeholder="Macro"
              />
