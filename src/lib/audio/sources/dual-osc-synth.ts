 ...
 * - 69 = A4 = 440 Hz
 * - formule standard : 440 * 2^((pitch - 69)/12)
 */
...
export class DualOscSynth {
  ...
  configure(opts: DualSynthParams): void {
    ...
    if (opts.mix !== undefined) {
      this.params.mix = this.clamp(opts.mix, 0, 1);
    }
    if (opts.detuneCents !== undefined) {
      this.params.detuneCents = this.clamp(opts.detuneCents, -2400, 2400);
    }
    if (opts.maxVoices !== undefined) {
      this.params.maxVoices = this.clamp(opts.maxVoices, 1, 64);
    }
    if (opts.envTarget) this.params.envTarget = opts.envTarget;
    if (opts.envDetuneDepth !== undefined) {
      this.params.envDetuneDepth = this.clamp(
        opts.envDetuneDepth,
        -2400,
        2400,
      );
    ...
    const gainA = Math.cos(mix * Math.PI * 0.5);
    const gainB = Math.sin(mix * Math.PI * 0.5);

    try {
      for (const v of this.voices) {
        if (v.active) {
          if (v.oscB) v.oscB.detune.value = this.params.detuneCents;
          v.mixGainA.gain.value = gainA;
          v.mixGainB.gain.value = gainB;
        }
      }
    } catch {}
  }
  ...
  private allocateVoice(destination: AudioNode): Voice | null {
    ...
    // 3) Voice stealing : voix active la plus ancienne (startedAt min)
    let idx = -1;
  ...
  noteOn(pitch: number, velocity: number, destination: AudioNode): void {
    ...
    if (detuneAMod && v.oscA) {
      const env = detuneAMod.envelope;
      ...
      const base = 0; // oscA n'utilise pas le detuneCents global
      const depth = detuneAMod.depthCents ?? 0;
      const v0 = base + depth * pts[0]!.value;

      try {
        v.oscA.detune.cancelScheduledValues(now);
      } catch {}
      v.oscA.detune.setValueAtTime(v0, now);
      ...
      for (let i = 0; i < pts.length - 1; i++) {
        ...
        const valA = base + depth * aPt.value;
        const valB = base + depth * bPt.value;
        const endT = Math.max(tA + 1e-4, tB);
        v.oscA.detune.setValueAtTime(valA, tA);
        v.oscA.detune.linearRampToValueAtTime(valB, endT);
      }
    }
    ...
    if (detuneMod && v.oscB) {
      ...
      try {
        v.oscB.detune.cancelScheduledValues(now);
      } catch {}
      v.oscB.detune.setValueAtTime(v0, now);
      ...
      for (let i = 0; i < pts.length - 1; i++) {
        ...
        const valB = base + depth * bPt.value;
        const endT = Math.max(tA + 1e-4, tB);
        v.oscB.detune.setValueAtTime(valA, tA);
        v.oscB.detune.linearRampToValueAtTime(valB, endT);
      }
    } else if (this.params.ampEnv && this.params.envTarget === "detune" && v.oscB) {
