...
class SessionPlayer {
  ...
  stopAll(): void {
    ...
    this._uiManager.clearAllPlaying().catch(() => {});
  }
  ...
  getActiveTrackIds(): string[] {
    ...
    const midiTracks: string[] = [];
    
    // Note: on devrait ajouter une méthode getActiveTrackIds() dans les managers
    // Pour l'instant on retourne un tableau vide, à améliorer
    return [...audioTracks, ...midiTracks];
  }

  // ==================== Helpers privés ====================

  private findClip(trackId: string, clipId: string): any {
    const proj = useProjectStore.getState().project;
    const scenes = proj.session?.scenes ?? [];
  ...
  private async getInstrumentConfig(trackId: string): Promise<any> {
    try {
  ...
  private async attachInstrumentSubscriptions(): Promise<void> {
    try {
      ...
      const applyToAll = async () => {
        const trackIds = this._midiManager.getMidiTrack ? [] : []; // À améliorer
  ...
  private async attachLoopWatcher(): Promise<void> {
    try {
      ...
      const cb = async () => {
        const proj = useProjectStore.getState().project;
        const playingCells = await this._uiManager.getPlayingCells();

        // Vérifier les boucles MIDI qui ne devraient plus l'être
        // (logique à implémenter selon besoins)
        
        // Vérifier les clips qui devraient passer en mode loop
        // (logique à implémenter selon besoins)
      };
