...
export function HiHatEditorModule({ trackId, className }: Props) {
  const [hat, setHat] = useState<HatParams>(() => drumMachine.getTrackPreset(trackId).hh);

  const update = (patch: Partial<HatParams>) => {
    setHat((h) => ({ ...h, ...patch }));
    drumMachine.setTrackPreset(trackId, { hh: patch });
  };
  ...
  const preview = async () => {
    ...
    // trigger hi-hat sound (drum-machine uses 'hh')
    drumMachine.playSound("hh", 120, when, trackId);
  };

  if (!hat) return null;

  const attackMs = hat.ampAttackMs ?? (hat.ampAttackSec ? hat.ampAttackSec * 1000 : 2);
  const decayMs = hat.ampDecayMs ?? (hat.ampDecaySec ? hat.ampDecaySec * 1000 : 60);

  return (
    <Card className={cn("flex flex-col gap-4 p-4 overflow-hidden", className)}>
      <div className="flex items-center justify-between">
        <div className="font-semibold leading-none">Hi-Hat</div>
        ...
        <div className="flex gap-3 p-0">
          <div className="rounded-xl border bg-background/40 p-2 space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              <Knob
                label="Noise dur ms"
                value={(hat.noiseDurSec ?? 0.06) * 1000}
                min={10}
                max={500}
                step={1}
...
export default HiHatEditorModule;
