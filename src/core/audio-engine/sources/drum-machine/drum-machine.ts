...
import { perfIncrementDrumTrigger } from "@/devtools/perf/audio-metrics"
import { DeepPartial, DrumInstrument, DrumPreset, TomStyle } from "./types"
import { MixerCore } from "../../core/mixer/mixer"
import { DEFAULT_PRESET } from "./presets"
import { triggerKick } from "./drums/kick/kick-dsp"
import { triggerSnare } from "./drums/snare/snare"
import { triggerHat } from "./drums/hi-hat/hi-hat"
import { triggerTom } from "./drums/tom/tom-dsp"
import { triggerCrash1, triggerCrash2, triggerRide, triggerRideBell, triggerSplash } from "./drums/crash/crash-dsp"
import { triggerChina } from "./drums/china/china-dsp"
import { AudioEngine, dbToGain } from "../../core/audio-engine"
import { InstrumentOutput, isTrackRoutingV2Enabled } from "../../core/instrument-output"
import { FxDecl } from "../../types"
...
export class DrumMachine {
  ...
  /** Stocke le handle du dernier kick joué pour gérer le choke (monophonie) */
  private lastKickHandle: Map<string, { stop: (t: number) => void }> = new Map()
  ...
  private playKick(ctx: AudioContext, ch: Channel, velocity: number, when: number, trackId: string) {
    // Choke group: couper le kick précédent s'il joue encore
    const last = this.lastKickHandle.get(trackId)
    if (last) {
      // FIX: On coupe 10ms AVANT le nouveau kick pour éviter tout chevauchement (overlap)
      // Cela garantit que le kick précédent est totalement silencieux quand le nouveau démarre.
      // Avec le lookahead de 500ms, 'when - 0.010' est toujours dans le futur.
      last.stop(when - 0.010)
    }

    const preset = this.ensurePreset(trackId)
    // triggerKick retourne maintenant un handle { stop }
    const handle = triggerKick(ctx, ch.gain, velocity, when, preset.kick)
    
    // On stocke le handle (si triggerKick retourne void pour l'instant, ça sera undefined, mais on a patché kick-dsp)
    if (handle) {
      this.lastKickHandle.set(trackId, handle)
    }
