...
import { clipManager, getAudioManager, getMidiManager } from "./session-player/clip-manager";
...
import { ClipDecl, MidiNote } from "../types";
...
class SessionPlayer {
  ...
  // Managers dédiés (via façade)
  private readonly _clipManager = clipManager;
  private readonly _audioManager = getAudioManager();
  private readonly _midiManager = getMidiManager();
  ...
  private async handleLaunch(
    ...
    try {
      ...
      MixerCore.ensure().ensureTrack(trackId);
  ...
  private async getInstrumentConfig(trackId: string): Promise<InstrumentConfig | null> {
    try {
      const { useInstrumentStore } = await import("@/features/daw/state/instrument.store");
      const kind = useInstrumentStore.getState().getKind(trackId);
      
      if (kind === "dual-synth") {
        const { useDualSynthStore } = await import("@/features/daw/state/dual-synth.store");
        return {
          kind: "dual-synth",
        ...
        const { useSynthStore } = await import("@/features/daw/state/synth.store");
        return {
          kind: "simple-synth",
  ...
  private async attachLoopWatcher(): Promise<void> {
    try {
      const { useProjectStore } = await import("@/features/daw/state/project.store");
